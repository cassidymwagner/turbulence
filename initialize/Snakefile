...
# download initial conditions in gamer format, created from external python script 
rule IC:
    input:
        'UM_IC',
...
rule template:
    run:
		...
		create_templates( {'driven_amp': float(amp)}, dest_path = ("initialize/templates/driven_amp" + str(amp).zfill(5)) ) 
